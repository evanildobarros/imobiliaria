...
<option value="">Selecione</option>
<?php
$qr178=mysql_query("SELECT * FROM cliente order by cliente asc");
...
<td class="font"><span class="span10"><?php echo $row['cliente']?></span><span class="span10"><?php echo $row['fornecedor']?></span> | <span class="span11"><?php echo $row['fpagamento']?></span> | <?php echo $row['descricao']?> <?php echo $row['desc']?> <em>(<a href="?mes=<?php echo $mes_hoje?>&ano=<?php echo $ano_hoje?>&filtro_cat=<?php echo $cat?>"><?php echo $categoria?></a>)</em> <a href="javascript:;" style="font-size:10px; color:#666" onclick="document.getElementById('editar_mov_<?php echo $row['id']?>').style.display='';  " title="Editar"><img width="17" height="17" src="../img/pen_write_edit.png" /></a> [<a href="recibo.php?id=<?php echo $row['id']; ?>">Recibo</a>]</td>

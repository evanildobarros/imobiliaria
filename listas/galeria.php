...
$colname_imovel = "-1";
if (isset($_GET['id_cliente'])) {
  $colname_imovel = $_GET['id_cliente'];
}
mysql_select_db($database_conexao, $conexao);
$query_imovel = sprintf("SELECT * FROM imovel WHERE id_cliente = %s", GetSQLValueString($colname_imovel, "text"));
$imovel = mysql_query($query_imovel, $conexao) or die(mysql_error());
$row_imovel = mysql_fetch_assoc($imovel);
$totalRows_imovel = mysql_num_rows($imovel);

$conexao = mysql_connect('localhost','root','jedai2003');
...
<body>
 <form name="upload_files" action="" enctype="multipart/form-data" method="post">
    <table width="526" border="1" align="center" bordercolor="#CCCCCC" style="border-collapse:collapse">
   <tr>
 ...
 $inserir = mysql_query("INSERT INTO galeria (img,id_cliente) VALUES ('$cod','$id_pro')");
